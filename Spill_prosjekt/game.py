...
ai_difficulty_box = Entity(model='cube', position=(.75,1.3,-1), scale=(.4,.2), color=color.gray)
ai_difficulty_text = Text(text="", position=(.15,.34), scale=(1.5,1.5), color=color.black)
ai_difficulty_box.visible = False
ai_difficulty_text.visible = False

  #dette definerer AI vanskelighets knappene
AI_off = Button(text="Off", scale=(.05,.05), position=(.4, -.3), color=color.gray, highlight_color=color.dark_gray, text_color=color.black)
difficulty_low = Button(text="Low", scale=(.05,.05), position=(.5,-.3), color=color.green, highlight_color=color.rgb(0, 120, 0), text_color=color.black)
difficulty_medium = Button(text="Medium", scale=(.1,.05), position=(.65,-.3), color=color.yellow, highlight_color=color.rgb(155, 135, 12), text_color=color.black)
difficulty_high = Button(text="High", scale=(.05,.05), position=(.8,-.3), color=color.red, highlight_color=color.rgb(139, 0, 0), text_color=color.black)
...
def update():
  ...
  if held_keys['right arrow']:
    player1.x += time.dt * 1.5
  if held_keys['left arrow']:
    player1.x -= time.dt * 1.5
  if held_keys['d']:
    player2.x += time.dt * 1.5
  if held_keys['a']:
    player2.x -= time.dt * 1.5
  ...
  if player1.x > 0.7:
    player1.x = player1.x - time.dt * 1.5
  if player1.x < -0.7:
    player1.x = player1.x + time.dt * 1.5
  if player2.x > 0.7:
    player2.x = player2.x - time.dt * 1.5
  if player2.x < -0.7:
    player2.x = player2.x + time.dt * 1.5
